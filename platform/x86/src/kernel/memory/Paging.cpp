...
void Kernel::bootstrapPaging(uint32_t *directory, uint32_t *biosDirectory) {
    ...
    auto *blockMap = (X86Platform::MemoryBlock*) ((uint32_t) (&X86Platform::blockMap) - KERNEL_START);
    ...
    for (uint32_t i = 0; blocksFound < 2; i++) {
        if (blockMap[i + 1].startAddress - (blockMap[i].startAddress + blockMap[i].blockCount * bigPageSize) >= bigPageSize || blockMap[i + 1].blockCount == 0) {
            ...
            if (blocksFound == 0) {
                ...
                blockMap[i + 1] = {physicalAddress, (kernelPage + pageCount) * bigPageSize, bigPageSize, 1, X86Platform::HEAP_RESERVED};
            } else {
                pagingAreaPhysicalAddress = physicalAddress;
                blockMap[i + 1] = {physicalAddress, VIRT_PAGE_MEM_START, bigPageSize, 1, X86Platform::PAGING_RESERVED};
            }
